...
public class Epic_Game
{
    public static void main(String[] args){
        ...
        if(r.equalsIgnoreCase(humanChoice.substring(humanChoice.length()-4))){
            human = 0;
        }else if(p.equalsIgnoreCase(humanChoice.substring(humanChoice.length()-5))){
            human = 1;
        }else if(s.equalsIgnoreCase(humanChoice.substring(humanChoice.length()-11))){
            human = 2;
        }
        ...
        if (humanChoice.length() >= 50 && human >= 0){
            ...
            System.out.print("Me: Does he know my strategy? Two stones in a row... what are the chances? Didhe steal my notes? Did he follow me into the archives? No, stay the course: ");
